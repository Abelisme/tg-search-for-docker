...
class Dumper:
    ...
    def __init__(self, config):
        ...
        if 'DBFileName' in self.config:
            ...
            self.conn = psycopg2.connect(database="tgapibot6",
                        host="localhost",
                        user="",
                        password="",
                        port="5432")
        ...
        self.dump_methods = ('message', 'userinfo', 'message_service', 'channel',
        ...
        c.execute("SELECT tablename FROM pg_catalog.pg_tables "
                  ...
                  "schemaname != 'information_schema' AND tablename='version'")
        ...
        if not exists:
            ...
            c.execute("INSERT INTO Version VALUES (%s)", (DB_VERSION,))

            c.execute("CREATE TABLE Forward("
                      "ID SERIAL PRIMARY KEY,"#SERIAL = autoincrease
    ...
    def check_self_user(self, self_id):
        ...
        if result:
            ...
            cur.execute("INSERT INTO SelfInformation VALUES (%s)", (self_id,))
            self.commit()
    ...
    def dump_participants_delta(self, context_id, ids):
        ...
        c.execute('SELECT Added, Removed FROM ChatParticipants '
                  'WHERE ContextID = %s ORDER BY DateUpdated ASC',
                  (context_id,))
        ...
        c.execute("INSERT INTO ChatParticipants VALUES (%s, %s, %s, %s)", row)
    ...
    def dump_media(self, media, media_type=None):
        ...
        if row['type']:
            ...
            c = self.conn.cursor()
            c.execute('SELECT ID FROM Media WHERE LocalID = %s '
                      'AND VolumeID = %s AND Secret = %s',
                      (row['local_id'], row['volume_id'], row['secret']))
            existing_row = c.fetchone()
            if existing_row:
                return existing_row[0]
            name = row['name'] if row['name'] != None else 'Null'
            mime_type = row['mime_type'] if row['mime_type'] != None else 'Null'
            # thumbnail_id = row['thumbnail_id'] if row['thumbnail_id'] != None else 1
            media_type = row['type'] if row['type'] != None else 'Null'
            local_id = row['local_id'] if row['local_id'] != None else 0
            volume_id = row['volume_id'] if row['volume_id'] != None else 0
            secret = row['secret'] if row['secret'] != None else 0
            extra = row['extra'] if row['extra'] != None else 'Null'
            return self._insert('Media(name, mimetype, size, thumbnailid, type, localid, volumeid, secret, extra)', (
                name, mime_type, row['size'],
                row['thumbnail_id'], media_type,
                local_id, volume_id, secret,
                extra
            ))
    ...
    def get_max_message_id(self, context_id):
        ...
        row = self.conn.execute("SELECT MAX(ID) FROM Message WHERE "
                                "ContextID = %s", (context_id,)).fetchone()
    ...
    def get_message_count(self, context_id):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM MESSAGE WHERE ContextID = %s", (context_id,)
        )
    ...
    def get_resume(self, context_id):
        ...
        """
        c = self.conn.cursor()
        c.execute("SELECT ID, Date, StopAt FROM Resume WHERE "
    ...
    def iter_resume_entities(self, context_id):
        ...
        """
        c = self.conn.cursor()
        c.execute("SELECT ID, AccessHash FROM ResumeEntity "
                              "WHERE ContextID = %s", (context_id,))
        row = c.fetchone()
        while row:
        ...
        c.execute("DELETE FROM ResumeEntity WHERE ContextID = %s",
                  (context_id,))
    ...
    def save_resume_entities(self, context_id, entities):
        ...
        c = self.conn.cursor()
        c.executemany("INSERT INTO ResumeEntity "
                      "VALUES (%s,%s,%s)", rows)
    ...
    def iter_resume_media(self, context_id):
        ...
        c.execute(
            ...
            "FROM ResumeMedia WHERE ContextID = %s", (context_id,)
        )
        row = c.fetchone()
        while row:
        ...
        c.execute("DELETE FROM ResumeMedia WHERE ContextID = %s",
                  (context_id,))
    ...
    def _insert_if_valid_date(self, into, values, date_column, where):
        ...
        As an example, ("ID", 4) -> WHERE ID = %s, 4
        """
        last = self.conn.execute(
            'SELECT * FROM {} WHERE {} = %s ORDER BY DateUpdated DESC'
    ...
    def _insert(self, into, values):
        ...
        try:
            ...
            fmt = ','.join(query_list)
            c = self.conn.cursor()
            sql_query = "INSERT INTO {} VALUES ({})".format(into, fmt)
